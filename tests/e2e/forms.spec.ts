...
test.describe('Contact Form', () => {
  ...
  test('form validation prevents empty submission', async ({ page }) => {
    ...
    await expect(page.locator('form').filter({ has: page.locator('[name="name"]') }).first()).toBeVisible();
  ...
  test('honeypot field is visually hidden from users', async ({ page }) => {
    ...
    // Honeypot is positioned off-screen
  ...
  test('successful submission shows success state', async ({ page }) => {
    ...
    await page.locator('[name="service"]').fill('Clinical Research');
